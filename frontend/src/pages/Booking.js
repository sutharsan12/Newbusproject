...
const Booking = () => {
  ...
  // }
  // setTempSeats(a ?? "");
  // console.log("tempSeats",a,selectedSeats);
  // },[selectedSeats])
...
for(let i =0;i<selectedSeats.length;i++){
    a=a+selectedSeats[i];
    if(i<selectedSeats.length-1) a+=",";
}
setSeatNumber(a ?? "");
        setBookingDate(date);
        setTotalFare(fare);
      }, [ selectedSeats, date, fare]);
  ...
  const generatePdf = () => {
    ...
    doc.text("Total Fare: " + totalFare, 10, 70);
    doc.text("Booking Date: " + bookingDate, 10, 80);
    doc.save("ticket.pdf");
  };
